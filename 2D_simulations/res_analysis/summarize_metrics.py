...
def base_summarize(npz, field="A_final", threshold="auto", layout="even-r", min_size=2):
    ...
    components = [c for c in component_mask(active_mask, layout) if len(c) >= min_size]  # Remove small components
    component_sizes = np.array([len(c) for c in components], dtype=float)
    diameters = np.array([approx_diameter_hexagons(int(s)) for s in component_sizes], dtype=float)
    ...
    if np.any(valid):
        ...
        peak_i = np.argmax(s)
        peak_wavelength = float(w[peak_i])
        peak_sharpness = float(np.max(s) / np.median(s))
    else:
        peak_wavelength = np.nan
        peak_sharpness = np.nan

    _, autocorr = analyze_image_autocorrelation(data)  # Autocorrelation
    corr = autocorr["radial_autocorrelation"].to_numpy()
    distance = autocorr["bin_centers"].to_numpy()

    zero_idx = np.where(corr <= 0)[0]  # Distance over which neighboring cells related
    zero_crossing = float(distance[zero_idx[0]]) if len(zero_idx) else np.nan

    decay_idx = np.where(corr <= 1 / np.e)[0]
    corr_length = float(distance[decay_idx[0]])
    size_cv = (float(component_sizes.std() / component_sizes.mean()))
    ...
    return {
        ...
        "mean_diameter_hexes": float(diameters.mean()),
