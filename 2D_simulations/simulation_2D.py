...
def run_coupled_hex(
    ...
    dense_save_until_step: int = 0,
    dense_save_every: int | None = None,
) -> tuple[list[np.ndarray], list[np.ndarray], int, float, float]:
    """
    Run the single-pair 2D hex-grid simulation.

    dense_save_until_step, dense_save_every
        Optional denser saving window for the early, fastest-changing part of
        the run. Before step `dense_save_until_step`, frames are saved every
        `dense_save_every` steps instead of every `save_every` steps. Both
        default to off (0 / None), which reproduces the exact original
    ...
    for step in range(steps):
        ...
        current_save_every = (
            dense_save_every
            if (step < dense_save_until_step and dense_save_every)
            else save_every
        )
        if step % current_save_every == 0:
            diff = (
    ...
    print(
        ...
        f"total average difference per tile over {current_save_every} steps = "
        f"{diff / (2 * Ny * Nx):.2e}"
    )
